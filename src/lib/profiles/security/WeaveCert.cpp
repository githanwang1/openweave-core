...
static WEAVE_ERROR EncodeWeaveDeviceId(TLVWriter& writer, uint64_t deviceId, uint8_t tag)
{
    WEAVE_ERROR err;
    TLVType containerType;

    err = writer.StartContainer(ContextTag(tag), kTLVType_Path, containerType);
    SuccessOrExit(err);
...
static WEAVE_ERROR EncodeCertKeyIdExtention(TLVWriter& writer, uint8_t *certKeyId, uint8_t certKeyIdLen, uint8_t tag)
{
    WEAVE_ERROR err;
    TLVType containerType;

    err = writer.StartContainer(ContextTag(tag), kTLVType_Structure, containerType);
    SuccessOrExit(err);

    err = writer.PutBytes(ContextTag(kTag_SubjectKeyIdentifier_KeyIdentifier), certKeyId, certKeyIdLen);
    SuccessOrExit(err);
                                                        ...
                                                        GenerateECDSASignatureFunct genCertSignature)
{
    ...
    err = EncodeWeaveDeviceId(writer, deviceId, kTag_Issuer);
    SuccessOrExit(err);
    ...
    err = EncodeWeaveDeviceId(writer, deviceId, kTag_Subject);
    SuccessOrExit(err);
    ...
    // Certificate key Id.
    {
        ...
        err = EncodeCertKeyIdExtention(writer, certKeyId, kCertKeyId_Length, kTag_SubjectKeyIdentifier);
        SuccessOrExit(err);

        // Certificate extension: authority key identifier.
        err = EncodeCertKeyIdExtention(writer, certKeyId, kCertKeyId_Length, kTag_AuthorityKeyIdentifier);
        SuccessOrExit(err);
    }
...

    {
        ...
        err = genCertSignature(certData->TBSHash, Platform::Security::SHA1::kHashLength, ecdsaSig);
        SuccessOrExit(err);
